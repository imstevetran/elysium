...
impl Parser {
    ...
    pub fn parse_program(&mut self) -> Result<Program> {
        ...
        while self.pos < self.tokens.len() {
            ...
            if self.peek_is(&Token::Let)
                ...
                || self.peek_is(&Token::Worker)
                || self.peek_is(&Token::Extension)
                || is_mod
            {
    ...
    fn parse_item(&mut self) -> Result<Node<Item>> {
        ...
        } else if self.peek_is(&Token::Extension) {
            self.parse_extension_stmt()
        } else {
    ...
    /// Parse: `extension "name" { keywords: [...], runtime: { ... } }`
    fn parse_extension_stmt(&mut self) -> Result<Node<Item>> {
        let start = self.advance_span_start(); // "extension"
        let (_, name) = self.expect_string()?;

        self.expect(&Token::LBrace)?;

        let mut keywords: Vec<String> = Vec::new();
        let mut runtime: Vec<RuntimeEntry> = Vec::new();

        // Parse optional block body: keywords: [...] and/or runtime: { ... }
        loop {
            ...
            match field.as_str() {
                ...
                "runtime" => {
                    ...
                    while !self.peek_is(&Token::RBrace) {
                        let platform = self.expect_identifier()?;
                        self.expect(&Token::Colon)?;
                        let (_, path) = self.expect_string()?;
                        runtime.push(RuntimeEntry { platform, path });
                        if self.peek_is(&Token::Comma) {
                            self.advance();
                        }
                    }
                ...
                _ => {
                    return Err(self.error(&format!("unknown field `{}` in extension block, expected `keywords` or `runtime`", field)));
                }
            }
        ...
        let (_, end) = self.last_span();
        Ok(Node::new(
            Item::Extension(ExtensionStmt { name, keywords, runtime }),
            crate::error::SourceSpan::new(start, end - start),
        ))
    }
    ...
    fn peek_is(&self, token: &Token) -> bool {
        ...
        match (current, token) {
            ...
            (Token::Extension, Token::Extension) => true,
    ...
    fn expect_token(&mut self, expected: &Token) -> Result<(usize, String)> {
        if self.pos < self.tokens.len() {
            let (start, ref tok, _end) = self.tokens[self.pos];
    ...
    fn token_eq(&self, a: &Token, b: &Token) -> bool {
        match (a, b) {
            ...
            | (Token::Worker, Token::Worker) | (Token::Extension, Token::Extension) | (Token::Is, Token::Is)
