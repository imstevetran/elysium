    ...
    let result = match &cli.command {
        ...
        cli::Commands::Test { path, dry_run, env } => {
            cmd_test(path, *dry_run, &resolve_env_alias(
                &path.as_ref().cloned().unwrap_or_else(|| PathBuf::from("core/examples/")),
                env,
            ))
        }
        ...
        cli::Commands::Init { name, description, version, author, repository } => {
            init::cmd_init(name, description.as_deref(), version, author.as_deref(), repository.as_deref())
        }
        cli::Commands::Install { package } => {
            install::cmd_install(package.as_deref())
        }
        cli::Commands::Publish { path } => {
            publish::cmd_publish(path.as_deref())
        }
...
fn compile_c_file(c_path: &Path) -> error::Result<PathBuf> {
...
fn build_elyx(file: &PathBuf, _output: Option<PathBuf>, _emit_ir: bool) -> error::Result<()> {
    let source = read_source(file)?;
