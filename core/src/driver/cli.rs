...
pub enum Commands {
    ...
    Test {
        /// Path to a .ely file or a directory containing .ely test files (default: core/examples/)
        path: Option<PathBuf>,
    ...
    /// Scaffold a new package directory in packages/
    Init {
        /// Package name (e.g. "my-pkg")
        name: String,
        /// Package description
        #[arg(long)]
    ...
    /// Install package dependencies from the EPM registry
    Install {
        /// Package name to install (omit to install all dependencies from elysium.json)
        package: Option<String>,
    },
    /// Publish a package to the EPM registry
    Publish {
