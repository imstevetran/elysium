...
/**
 * EPM (Elysium Package Manager) CLI launcher.
 *
...
const BIN_NAME = process.platform === 'win32' ? 'epm.exe' : 'epm';
const BIN_PATH = path.join(BIN_DIR, BIN_NAME);

if (!fs.existsSync(BIN_PATH)) {
  console.error('EPM binary not found. Try running `npm run postinstall` to rebuild.');
...
if (result.error) {
  console.error(`Failed to run EPM: ${result.error.message}`);
  process.exit(1);
}
